...
import type { ComputerOrderResponse, CreateComputerOrderRequest } from "../Types/ComputerTypes";
...
export async function CreateComputerOrder(formData: CreateComputerOrderRequest, user: User) {
...
export async function GetComputerOrders(user: User) {
  return await axios.get<ComputerOrderResponse[]>("http://localhost:5268/computer-orders", {
    headers: { Authorization: `Bearer ${user.token}` },
  });
}
...
export async function GetComputerOrderById(id: number, user: User) {
  return await axios.get<ComputerOrderResponse>(`http://localhost:5268/computer-orders/${id}`, {
    headers: { Authorization: `Bearer ${user.token}` },
  });
}
...
export async function MakeDecisionAsApprover(id: number, decision: boolean, user: User) {
  return await axios.put(`http://localhost:5268/computer-orders/approval/${id}`, decision, {
    headers: {
